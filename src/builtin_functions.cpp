                    ...
                    [this](LLVMBuilderRef builder, const std::vector<LLVMValueRef> &args) -> LLVMValueRef
                    {
                        ...
                        if (LLVMGetTypeKind(ty) == LLVMIntegerTypeKind)
                        {
                            ...
                            if (w == 32)
                                return v;
                            if (w > 32)
                                return LLVMBuildTrunc(builder_, v, int32_t_, "trunc_i32");
...
void BuiltinFunctions::registerArrayFunctions()
{
    ...
    registerBuiltin("array_slice", i8p, {i8p, int32_t_, int32_t_, int32_t_}, false,
                    [this, i8p](LLVMBuilderRef builder, const std::vector<LLVMValueRef> &args)
                    {
                        ...
                        {
                            LLVMValueRef vals[] = {zero};
                            LLVMBasicBlockRef blks[] = {fixS};
                            LLVMAddIncoming(sPhi, vals, blks, 1);
                        }
                        {
                            LLVMValueRef vals[] = {start};
                            LLVMBasicBlockRef blks[] = {cur};
                            LLVMAddIncoming(sPhi, vals, blks, 1);
                        }
                        ...
                        {
                            LLVMValueRef vals[] = {len};
                            LLVMBasicBlockRef blks[] = {fixE};
                            LLVMAddIncoming(ePhi, vals, blks, 1);
                        }
                        {
                            LLVMValueRef vals[] = {end};
                            LLVMBasicBlockRef blks[] = {contS};
                            LLVMAddIncoming(ePhi, vals, blks, 1);
                        }
...
void BuiltinFunctions::registerMathFunctions()
{
    ...
    registerBuiltin("abs_f64", double_t_, {double_t_}, false,
                    [this](LLVMBuilderRef builder, const std::vector<LLVMValueRef> &args)
                    {
                        ...
                        {
                            if (xTy != int32_t_)
                                x = LLVMBuildZExt(builder_, x, int32_t_, "to_i32");
    ...
    registerBuiltin("min_f64", double_t_, {double_t_, double_t_}, false,
                    [this](LLVMBuilderRef builder, const std::vector<LLVMValueRef> &args)
                    {
                        ...
                        {
                            if (aTy != int32_t_)
                                a = LLVMBuildZExt(builder_, a, int32_t_, "a_i32");
                        ...
                        {
                            if (bTy != int32_t_)
                                b = LLVMBuildZExt(builder_, b, int32_t_, "b_i32");
    ...
    registerBuiltin("min", double_t_, {}, /*isVariadic*/ true,
                ...
                for (auto v : av) if (LLVMGetTypeKind(LLVMTypeOf(v)) != LLVMIntegerTypeKind) return false; return true; };
                        if (allInt(args))
                        {
                            ...
                            { return (LLVMTypeOf(v) == int32_t_) ? v : LLVMBuildZExt(builder_, v, int32_t_, "i32"); };
                            a = toI32(a);
                            b = toI32(b);
                        ...
                        { LLVMTypeRef ty = LLVMTypeOf(v); if (LLVMGetTypeKind(ty) == LLVMIntegerTypeKind) { if (ty != int32_t_) v = LLVMBuildZExt(builder_, v, int32_t_, "i32"); return LLVMBuildSIToFP(builder_, v, double_t_, "f64"); } if (LLVMGetTypeKind(ty) == LLVMFloatTypeKind) return LLVMBuildFPExt(builder_, v, double_t_, "f64"); return v; };
    ...
    registerBuiltin("clamp_f64", double_t_, {double_t_, double_t_, double_t_}, false,
                    [this](LLVMBuilderRef builder, const std::vector<LLVMValueRef> &args)
                    {
                        ...
                        auto toF64 = [&](LLVMValueRef v, const char *name)
                        {
                            ...
                            {
                                if (ty != int32_t_)
                                    v = LLVMBuildZExt(builder_, v, int32_t_, (std::string(name) + "_i32").c_str());
    ...
    registerBuiltin("clamp", /*returnType placeholder*/ double_t_, {}, /*isVariadic*/ true,
                    [this](LLVMBuilderRef builder, const std::vector<LLVMValueRef> &args)
                    {
                        ...
                        if (isAllInt(args))
                        {
                            ...
                            {
                                LLVMTypeRef ty = LLVMTypeOf(v);
                                if (LLVMGetTypeKind(ty) == LLVMIntegerTypeKind && ty != int32_t_)
                                    return LLVMBuildZExt(builder_, v, int32_t_, "i32");
                                if (LLVMGetTypeKind(ty) == LLVMPointerTypeKind)
                                    return LLVMBuildPtrToInt(builder_, v, int32_t_, "i32ptr");
                        ...
                        auto toF64 = [&](LLVMValueRef v)
                        {
                            ...
                            {
                                if (ty != int32_t_)
                                    v = LLVMBuildZExt(builder_, v, int32_t_, "i32");
...
void BuiltinFunctions::registerTypesWithExpressionCodeGen(ExpressionCodeGen *exprGen)
{
    ...
    for (const auto &pair : builtins_)
    {
        ...
        if (bi->returnType == int32_t_)
            qt = QuarkType::Int;
        else if (bi->returnType == float_t_)
            qt = QuarkType::Float;
        else if (bi->returnType == double_t_)
            qt = QuarkType::Double;
        else if (bi->returnType == int8ptr_t_)
            qt = QuarkType::String;
        else if (bi->returnType == bool_t_)
            qt = QuarkType::Boolean;
        else if (bi->returnType == LLVMVoidTypeInContext(ctx_))
            qt = QuarkType::Void;
...
{
    if (args.size() != 2)
        return LLVMConstInt(bool_t_, 0, 0);
