...
std::optional<std::vector<uint8_t>> CompilationCache::getCachedBitcode(const std::string &sourcePath) const
{
    ...
    if (!std::filesystem::exists(bitcodePath))
        return std::nullopt;
...
void CompilationCache::loadManifest()
{
    ...
    auto findValue = [&content](const std::string &key, size_t start = 0) -> std::pair<std::string, size_t>
    {
        ...
        else if (content[valueStart] == '{' || content[valueStart] == '[')
        {
            ...
            {
                if (content[valueEnd] == openChar)
                    ++depth;
                else if (content[valueEnd] == closeChar)
                    --depth;
                ++valueEnd;
            }
    ...
    if (entriesJson.empty() || entriesJson[0] != '{')
        return;
    ...
    while (pos < entriesJson.size())
    {
        ...
        {
            if (entriesJson[objEnd] == '{')
                ++depth;
            else if (entriesJson[objEnd] == '}')
                --depth;
            ++objEnd;
        }
        ...
        auto extractField = [&entryJson](const std::string &field) -> std::string
        {
            ...
            if (entryJson[valStart] == '"')
            {
                ...
                while (valEnd < entryJson.size() && !(entryJson[valEnd] == '"' && entryJson[valEnd - 1] != '\\'))
                    ++valEnd;
        ...
        if (!optLevel.empty())
        {
            try
            {
                entry.optimizationLevel = std::stoi(optLevel);
            }
            catch (...)
            {
            }
                                     ...
                                     bool freestanding) const
{
    if (entry.sourceHash != currentHash)
        return false;
    if (entry.optimizationLevel != optimizationLevel)
        return false;
    if (entry.freestanding != freestanding)
        return false;
