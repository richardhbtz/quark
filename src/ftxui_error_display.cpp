...
Element FTXUIErrorDisplay::highlightSourceLine(const std::string &line, bool isErrorLine)
{
    ...
    while (pos < remaining.length())
    {
        ...
        {
            size_t start = pos;
            while (pos < remaining.length() && (std::isdigit(remaining[pos]) || remaining[pos] == '.' || remaining[pos] == 'x'))
                pos++;
        ...
        {
            size_t start = pos;
            while (pos < remaining.length() && (std::isalnum(remaining[pos]) || remaining[pos] == '_'))
                pos++;
