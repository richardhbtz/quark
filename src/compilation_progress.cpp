...
void CompilationProgress::start(const std::string &filename)
{
    ...
    {
        std::cout << std::endl;
        renderThread_ = std::make_unique<std::thread>([this]()
                                                      { renderLoop(); });
    }
}
...
void CompilationProgress::setStage(Stage stage)
{
    if (currentStage_ != stage)
    {
        ...
        {
        case Stage::LEXING:
            overallProgress_ = 0.0f;
            break;
        case Stage::PARSING:
            overallProgress_ = 0.2f;
            break;
        case Stage::CODE_GENERATION:
            overallProgress_ = 0.4f;
            break;
        case Stage::OPTIMIZATION:
            overallProgress_ = 0.7f;
            break;
        case Stage::LINKING:
            overallProgress_ = 0.85f;
            break;
        case Stage::COMPLETE:
            overallProgress_ = 1.0f;
            break;
        }
    }
}
...
Element CompilationProgress::createProgressBar(float progress)
{
    ...
    return hbox({text(bar) | color(Color::Cyan),
                 text(" " + oss.str()) | bold});
}
