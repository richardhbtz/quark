...
void StatementCodeGen::genFunctionStmt(FunctionAST* f, LLVMValueRef putsFn)
{
    ...
    LLVMValueRef fn = it->second;
        ...
        if (verbose_) printf("[codegen] function '%s' already has a body; skipping generation\n", f->name.c_str());
        return;
    }
    ...
    if (isStructMethod && !isConstructor) {
        ...
        if (structIt != g_struct_types_->end()) {
            ...
            (*g_named_values_)["this"] = thisAlloca;             (*g_named_types_)["this"] = selfPtrTy;
        ...
        unsigned llvmParamIndex = paramIndex;     for (unsigned i = 0; i < f->params.size(); ++i)
    {
        ...
        if (ptype == "int") paramType = LLVMInt32TypeInContext(ctx_);
        else if (ptype == "float") paramType = float_t_;
        else if (ptype == "double") paramType = double_t_;
        else if (ptype == "str") paramType = int8ptr_t_;
        else if (ptype == "bool") paramType = bool_t_;
        ...
        if (ptype == "int") (*g_named_types_)[pname] = LLVMInt32TypeInContext(ctx_);
        else if (ptype == "float") (*g_named_types_)[pname] = float_t_;
        else if (ptype == "double") (*g_named_types_)[pname] = double_t_;
        else if (ptype == "str") (*g_named_types_)[pname] = int8ptr_t_;
        else if (ptype == "bool") (*g_named_types_)[pname] = bool_t_;
        else if (!ptype.empty() && ptype.find('*') != std::string::npos) {
            ...
            if (elementType == "str") elementQuarkType = QuarkType::String;
            else if (elementType == "int") elementQuarkType = QuarkType::Int;
            else if (elementType == "float") elementQuarkType = QuarkType::Float;
            else if (elementType == "double") elementQuarkType = QuarkType::Double;
            else if (elementType == "bool") elementQuarkType = QuarkType::Boolean;
    ...
    LLVMBasicBlockRef epilogueBB = LLVMAppendBasicBlockInContext(ctx_, fn, "func_epilogue");
        ...
        if (bbIt == epilogueBB) continue; // skip epilogue itself
...
void StatementCodeGen::genVarDeclStmt(VarDeclStmt* vdecl)
{
            ...
            if (!bothIntegers && !bothNumeric) {
                auto typeToStr = [](QuarkType t) {
                    ...
                    if (IntegerTypeUtils::isFloatingType(t)) {
                        if (t == QuarkType::Float) return std::string("float");
                        if (t == QuarkType::Double) return std::string("double");
                    }
    ...
    switch (declaredType) {
        ...
        case QuarkType::Array:
            actualType = vdecl->type;                                     if (initType.elementType == QuarkType::Int) {
...
void StatementCodeGen::genAssignStmt(AssignStmtAST* as)
{
    ...
    (*g_named_types_)[as->varName] = varType;
        ...
        if (pointerTypeNameForValue.empty()) pointerTypeNameForValue = valueType.pointerTypeName;
        if (pointerTypeNameForValue.empty()) pointerTypeNameForValue = "void*";
...
{
    if (verbose_) printf("[DEBUG] collectFunctions: called with statement type\n");
    ...
    if (auto *f = dynamic_cast<FunctionAST *>(stmt))
    {
    if (verbose_) printf("[DEBUG] collectFunctions: found regular function: %s\n", f->name.c_str());
        if (verbose_)
    ...
    {
    if (verbose_) printf("[DEBUG] collectFunctions: found struct definition %s with %zu methods\n", structDef->name.c_str(), structDef->methods.size());
        ...
        for (auto &method : structDef->methods)
        {
            ...
            if (verbose_) printf("[DEBUG] collectFunctions: adding method %s to functions vector\n", method->name.c_str());
            if (verbose_)
    ...
    {
    if (verbose_) printf("[DEBUG] collectFunctions: found impl statement\n");
        if (verbose_)
    ...
    {
    if (verbose_) printf("[DEBUG] collectFunctions: found include statement\n");
        if (verbose_)
    ...
    {
    if (verbose_) printf("[DEBUG] collectFunctions: statement is not a function, struct, impl, or include\n");
        if (verbose_)
...
void StatementCodeGen::predeclareExternStructs(StmtAST* stmt)
{
    ...
    if (auto* inc = dynamic_cast<IncludeStmt*>(stmt)) {
        for (auto& s : inc->stmts) predeclareExternStructs(s.get());
        return;
    }
}
...
void StatementCodeGen::declareExternFunction(ExternFunctionAST* externFunc)
{
    ...
    if (verbose_)
        ...
        auto mapType = [this](const std::string& type) -> LLVMTypeRef {
            ...
            if (elem == "int") return LLVMPointerType(int32_t_, 0);
            if (elem == "float") return LLVMPointerType(float_t_, 0);
            if (elem == "double") return LLVMPointerType(double_t_, 0);
            if (elem == "str") return LLVMPointerType(int8ptr_t_, 0); // char**
            if (elem == "bool") return LLVMPointerType(bool_t_, 0);
        ...
        else if (type == "float") return LLVMFloatTypeInContext(ctx_);
        else if (type == "double") return LLVMDoubleTypeInContext(ctx_);
        else if (type == "str" || type == "str*") return int8ptr_t_;
        ...
        else if (type == "str" || type == "str*" || type == "char*") return QuarkType::String;
        else if (type == "bool") return QuarkType::Boolean;
        else if (type == "void") return QuarkType::Void;
        else if (type == "void*") return QuarkType::Pointer;
        else if (type == "char") return QuarkType::Int;
...
void StatementCodeGen::collectStructDefs(StmtAST* stmt, std::vector<StructDefStmt*>& structDefs)
{
    ...
    if (auto* structDef = dynamic_cast<StructDefStmt*>(stmt)) {
        ...
        structDefs.push_back(structDef);
                ...
                if (verbose_) printf("[codegen] collectStructDefs: descending into include with %zu stmts\n", includeStmt->stmts.size());
...
void StatementCodeGen::genWhileStmt(WhileStmt* whileStmt, LLVMValueRef putsFn)
{
    ...
    if (!currentFn) {
        ...
        if (verbose_) printf("[codegen] completed while statement (folded constant condition)\n");
        return;
    }
