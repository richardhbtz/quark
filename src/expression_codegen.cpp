...
TypeInfo ExpressionCodeGen::inferType(ExprAST *expr)
{
        ...
        if (auto *varExpr = dynamic_cast<VariableExprAST *>(memberAccess->object.get()))
        {
            ...
            for (const auto &field : allFields)
            {
                if (field.first == memberAccess->fieldName)
                {
                    ...
                    if (field.second == "int")
                        fieldType = QuarkType::Int;
                    else if (field.second == "float")
                        fieldType = QuarkType::Float;
                    else if (field.second == "double")
                        fieldType = QuarkType::Double;
                    else if (field.second == "str")
                        fieldType = QuarkType::String;
                    else if (field.second == "bool")
                        fieldType = QuarkType::Boolean;
                    else if (field.second.size() > 2 && field.second.substr(field.second.size() - 2) == "[]")
                    {
                        ...
                        if (elementTypeName == "str")
                            elementType = QuarkType::String;
                        else if (elementTypeName == "int")
                            elementType = QuarkType::Int;
                        else if (elementTypeName == "float")
                            elementType = QuarkType::Float;
                        else if (elementTypeName == "double")
                            elementType = QuarkType::Double;
                        else if (elementTypeName == "bool")
                            elementType = QuarkType::Boolean;
    ...
    if (auto *funcRef = dynamic_cast<FunctionRefExpr *>(expr))
    {
        if (g_function_map_ && g_function_map_->find(funcRef->functionName) != g_function_map_->end())
        {
            ...
            {
                auto &funcType = functionTypes_[funcRef->functionName];
                fpInfo->returnType = IntegerTypeUtils::quarkTypeToString(funcType.type);
            }
            else
            {
            ...
            if (g_function_param_types_ && g_function_param_types_->find(funcRef->functionName) != g_function_param_types_->end())
            {
                ...
                for (auto paramTy : paramTypes)
                {
                    ...
                    if (paramTy == int32_t_)
                        ...
                        fpInfo->paramTypes.push_back("float");
                    else if (paramTy == double_t_)
                        fpInfo->paramTypes.push_back("double");
                    else if (paramTy == bool_t_)
                        fpInfo->paramTypes.push_back("bool");
        ...
        return TypeInfo(QuarkType::FunctionPointer, expr->location);
    }
    ...
    if (auto *addrOf = dynamic_cast<AddressOfExpr *>(expr))
    {
        ...
        if (auto *var = dynamic_cast<VariableExprAST *>(addrOf->operand.get()))
        {
            if (g_function_map_ && g_function_map_->find(var->name) != g_function_map_->end())
            {
                ...
                {
                    auto &funcType = functionTypes_[var->name];
                    fpInfo->returnType = IntegerTypeUtils::quarkTypeToString(funcType.type);
                }
                else
                {
                ...
                if (g_function_param_types_ && g_function_param_types_->find(var->name) != g_function_param_types_->end())
                {
                    auto &paramTypes = (*g_function_param_types_)[var->name];
                    for (auto paramTy : paramTypes)
                    {
                        if (paramTy == int32_t_)
                            fpInfo->paramTypes.push_back("int");
    ...
    if (auto *cast = dynamic_cast<CastExpr *>(expr))
    {
        ...
        if (cast->targetTypeName == "int")
            return TypeInfo(QuarkType::Int, expr->location);
        if (cast->targetTypeName == "float")
            return TypeInfo(QuarkType::Float, expr->location);
        if (cast->targetTypeName == "double")
            return TypeInfo(QuarkType::Double, expr->location);
        if (cast->targetTypeName == "bool")
            return TypeInfo(QuarkType::Boolean, expr->location);
        if (cast->targetTypeName == "str")
            return TypeInfo(QuarkType::String, expr->location);
    ...
    if (auto *v = dynamic_cast<VariableExprAST *>(expr))
    {
        ...
        if (g_named_types_)
        {
            ...
            if (git != g_named_types_->end())
            {
                ...
                if (git->second == int32_t_)
                    type = QuarkType::Int;
                else if (git->second == int8ptr_t_)
                    type = QuarkType::String;
                else if (git->second == bool_t_)
                    type = QuarkType::Boolean;
        ...
        if (objType.type == QuarkType::Struct)
        {
            ...
            if (!actualStructType.empty())
            {
                ...
                while (findIt == functionTypes_.end())
                {
                    ...
                    if (defIt == g_struct_defs_->end() || defIt->second->parentName.empty())
                        break;
            ...
            if (bi)
            {
                if (call->callee == "clamp" || call->callee == "min" || call->callee == "max")
                {
                    ...
                    for (const auto &arg : call->args)
                    {
                        ...
                        if (!IntegerTypeUtils::isIntegerType(t.type))
                        {
                            allInt = false;
                            break;
                        }
...
void ExpressionCodeGen::declareVariable(const std::string &name, const TypeInfo &typeInfo)
{
    variableTypes_[name] = typeInfo;
}
...
std::optional<std::string> ExpressionCodeGen::evalConstString(ExprAST *expr)
{
    ...
    {
        char buf[64];
        snprintf(buf, sizeof(buf), "%g", static_cast<double>(f->value));
        return std::string(buf);
    }
    if (auto *b = dynamic_cast<BinaryExprAST *>(expr))
    {
...
LLVMValueRef ExpressionCodeGen::genExpr(ExprAST *expr)
{
    if (auto *cast = dynamic_cast<CastExpr *>(expr))
    {
        ...
        try
        {
            srcT = inferType(cast->operand.get());
        }
        catch (...)
        {
            srcT = TypeInfo(QuarkType::Unknown, expr->location);
        }
        ...
        if (dstQt == QuarkType::Double)
        {
            if (srcT.type == QuarkType::Double)
                return val;
            if (srcT.type == QuarkType::Float)
                return LLVMBuildFPExt(builder_, val, double_t_, "float_to_double");
            if (IntegerTypeUtils::isIntegerType(srcT.type))
            {
                LLVMValueRef i32v = (LLVMTypeOf(val) == int32_t_) ? val : LLVMBuildZExt(builder_, val, int32_t_, "to_i32");
    ...
    if (auto *funcRef = dynamic_cast<FunctionRefExpr *>(expr))
    {
        if (g_function_map_)
        {
            auto fit = g_function_map_->find(funcRef->functionName);
            if (fit != g_function_map_->end())
            {
                if (verbose_)
                    printf("[codegen] returning function reference: %s\n", funcRef->functionName.c_str());
                return fit->second;
            }
        }
    ...
    if (auto *b = dynamic_cast<BinaryExprAST *>(expr))
    {
        ...
        if (IntegerTypeUtils::isFloatingType(lhsType.type) || IntegerTypeUtils::isFloatingType(rhsType.type))
        {
            ...
            {
            case '-':
                return LLVMBuildFSub(builder_, lhs, rhs, "subtmp");
            case '*':
                return LLVMBuildFMul(builder_, lhs, rhs, "multmp");
            case '/':
                return LLVMBuildFDiv(builder_, lhs, rhs, "divtmp");
            case '%':
            {
                LLVMValueRef fmodFn;
        ...
        else
        {
            ...
            {
            case '-':
                return LLVMBuildSub(builder_, lhs, rhs, "subtmp");
            case '*':
                return LLVMBuildMul(builder_, lhs, rhs, "multmp");
            case '/':
                return LLVMBuildSDiv(builder_, lhs, rhs, "divtmp");
            case '%':
                return LLVMBuildSRem(builder_, lhs, rhs, "modtmp");
            default:
                throw std::runtime_error("unsupported binary op");
            }
        }
                    ...
                    if (typeIt != g_named_types_->end())
                    {
                                 ...
                                 LLVMGetElementType(typeIt->second) == LLVMInt8TypeInContext(ctx_))
                        {
                            ...
                            LLVMValueRef indices[] = {
                                ...
                                LLVMConstInt(LLVMInt64TypeInContext(ctx_), 0, false)};
                            return LLVMBuildGEP2(builder_, typeIt->second, it->second, indices, 2, (v->name + ".ptr").c_str());
                        }
                ...
                if (g_named_values_ && g_named_values_->find(c->callee) != g_named_values_->end())
                {
                    ...
                    if (calleeType.type == QuarkType::FunctionPointer || calleeType.type == QuarkType::Pointer)
                    {
                        ...
                        if (calleeType.funcPtrInfo)
                        {
                            ...
                            else if (retTypeStr == "int")
                            {
                                returnType = int32_t_;
                            }
                            else if (retTypeStr == "float")
                            {
                                returnType = float_t_;
                            }
                            ...
                            else if (retTypeStr == "str")
                            {
                                returnType = int8ptr_t_;
                            }

                            for (const auto &paramTypeStr : calleeType.funcPtrInfo->paramTypes)
                            {
                                if (paramTypeStr == "int")
                                {
            ...
            if (g_function_param_types_)
            {
                ...
                if (paramTypesIt != g_function_param_types_->end())
                {
                            ...
                            if (er && sm)
                            {
                                ...
                                if (file)
                                {
                                    throw EnhancedCodeGenError("variadic function '" + c->callee + "' expects at least " +
                                                               ...
                                                               c->location, file->content, ErrorCodes::INVALID_SYNTAX, (int)std::max<size_t>(1, c->callee.size()));
                                }
                            }
                    ...
                    for (size_t i = 0; i < argsToCheck; ++i)
                    {
                        ...
                        {
                            if (argType.type == QuarkType::Unknown)
                                continue;
                            if (expectedQuarkType == QuarkType::String && argType.type == QuarkType::Array)
                                continue;
                            if (expectedQuarkType == QuarkType::String && argType.type == QuarkType::Pointer)
                                continue;
                            if (expectedQuarkType == QuarkType::Array && argType.type == QuarkType::Pointer)
                                continue;
...
LLVMValueRef ExpressionCodeGen::genExprInt(ExprAST *expr)
{
    ...
    if (auto *v = dynamic_cast<VariableExprAST *>(expr))
    {
        ...
        if (LLVMGetTypeKind(valType) == LLVMIntegerTypeKind)
        {
            ...
            if (w == 32)
                return val;
            if (w > 32)
                return LLVMBuildTrunc(builder_, val, LLVMInt32TypeInContext(ctx_), "trunc_to_i32");
    ...
    {
        if (verbose_)
            printf("[codegen] generating dereference expression in integer context\n");
        ...
        if (verbose_)
            printf("[codegen] dereferenced pointer for integer\n");
        return result;
    }
    ...
    if (auto *b = dynamic_cast<BinaryExprAST *>(expr))
    {
        ...
        {
        case '+':
            return LLVMBuildAdd(builder_, L, R, "addtmp");
        case '-':
            return LLVMBuildSub(builder_, L, R, "subtmp");
        case '*':
            return LLVMBuildMul(builder_, L, R, "multmp");
        case '/':
        ...
        if (b->op == '=' || b->op == 'n' || b->op == '<' || b->op == '>' || b->op == 'l' || b->op == 'g')
        {
            ...
            try
            {
                    ...
                    (rhsType.type == QuarkType::Pointer || rhsType.type == QuarkType::Null))
                {
                    ...
                    {
                    case '=':
                        return LLVMBuildICmp(builder_, LLVMIntEQ, Lint, Rint, "eqptr");
                    case 'n':
                        return LLVMBuildICmp(builder_, LLVMIntNE, Lint, Rint, "neptr");
                    default:
            ...
            if (g_named_types_)
            {
                ...
                if (!lhsTypeKnown && lhsNamedString)
                    isStringComparison = true;
                if (!rhsTypeKnown && rhsNamedString)
                    isStringComparison = true;
            }
            ...
            if (isStringComparison)
            {
                ...
                {
                case '=':
                    return LLVMBuildICmp(builder_, LLVMIntEQ, cmpResult, LLVMConstInt(LLVMInt32TypeInContext(ctx_), 0, 0), "eqtmp");
                case 'n':
                    return LLVMBuildICmp(builder_, LLVMIntNE, cmpResult, LLVMConstInt(LLVMInt32TypeInContext(ctx_), 0, 0), "netmp");
                case '<':
                    return LLVMBuildICmp(builder_, LLVMIntSLT, cmpResult, LLVMConstInt(LLVMInt32TypeInContext(ctx_), 0, 0), "lttmp");
                case '>':
                    return LLVMBuildICmp(builder_, LLVMIntSGT, cmpResult, LLVMConstInt(LLVMInt32TypeInContext(ctx_), 0, 0), "gttmp");
                case 'l':
                    return LLVMBuildICmp(builder_, LLVMIntSLE, cmpResult, LLVMConstInt(LLVMInt32TypeInContext(ctx_), 0, 0), "letmp");
                case 'g':
                    return LLVMBuildICmp(builder_, LLVMIntSGE, cmpResult, LLVMConstInt(LLVMInt32TypeInContext(ctx_), 0, 0), "getmp");
                default:
            ...
            else
            {
                if (isBooleanComparison)
                {
                    ...
                    {
                    case '=':
                        return LLVMBuildICmp(builder_, LLVMIntEQ, L, R, "eqbool");
                    case 'n':
                        return LLVMBuildICmp(builder_, LLVMIntNE, L, R, "nebool");
                    case '<':
                        return LLVMBuildICmp(builder_, LLVMIntULT, L, R, "ltbool");
                    case '>':
                        return LLVMBuildICmp(builder_, LLVMIntUGT, L, R, "gtbool");
                    case 'l':
                        return LLVMBuildICmp(builder_, LLVMIntULE, L, R, "lebool");
                    case 'g':
                        return LLVMBuildICmp(builder_, LLVMIntUGE, L, R, "gebool");
                    default:
                        throw std::runtime_error("unknown comparison operator");
                    }
                }
                ...
                if (isFloatComparison)
                {
                    ...
                    if (LLVMTypeOf(L) != targetType)
                    {
                        if (LLVMGetTypeKind(lType) == LLVMIntegerTypeKind)
                        {
                            L = LLVMBuildSIToFP(builder_, L, targetType, "int_to_fp");
                        }
                        else if (lType == float_t_ && useDouble)
                        {
                            L = LLVMBuildFPExt(builder_, L, double_t_, "fp_ext");
                        }
                    }
                    if (LLVMTypeOf(R) != targetType)
                    {
                        if (LLVMGetTypeKind(rType) == LLVMIntegerTypeKind)
                        {
                            R = LLVMBuildSIToFP(builder_, R, targetType, "int_to_fp");
                        }
                        else if (rType == float_t_ && useDouble)
                        {
                            R = LLVMBuildFPExt(builder_, R, double_t_, "fp_ext");
                        }
                    }
                    ...
                    {
                    case '=':
                        return LLVMBuildFCmp(builder_, LLVMRealOEQ, L, R, "eqfloat");
                    case 'n':
                        return LLVMBuildFCmp(builder_, LLVMRealONE, L, R, "nefloat");
                    case '<':
                        return LLVMBuildFCmp(builder_, LLVMRealOLT, L, R, "ltfloat");
                    case '>':
                        return LLVMBuildFCmp(builder_, LLVMRealOGT, L, R, "gtfloat");
                    case 'l':
                        return LLVMBuildFCmp(builder_, LLVMRealOLE, L, R, "lefloat");
                    case 'g':
                        return LLVMBuildFCmp(builder_, LLVMRealOGE, L, R, "gefloat");
                    default:
                        throw std::runtime_error("unknown comparison operator");
                    }
                }
                ...
                {
                case '=':
                    return LLVMBuildICmp(builder_, LLVMIntEQ, L, R, "eqtmp");
                case 'n':
                    return LLVMBuildICmp(builder_, LLVMIntNE, L, R, "netmp");
                case '<':
                    return LLVMBuildICmp(builder_, LLVMIntSLT, L, R, "lttmp");
                case '>':
                    return LLVMBuildICmp(builder_, LLVMIntSGT, L, R, "gttmp");
                case 'l':
                    return LLVMBuildICmp(builder_, LLVMIntSLE, L, R, "letmp");
                case 'g':
                    return LLVMBuildICmp(builder_, LLVMIntSGE, L, R, "getmp");
                default:
                    throw std::runtime_error("unknown comparison operator");
                }
            }
            ...
            if (auto *objVar = dynamic_cast<VariableExprAST *>(memberAccess->object.get()))
            {
                ...
                else
                {
                    ...
                    if (LLVMGetTypeKind(spTy2) == LLVMStructTypeKind)
                    {
                        ...
                        if (verbose_)
                            printf("[expression_codegen] genMemberAccess: stored struct value into alloca for '%s'\n", objVar->name.c_str());
                    }
                    if (LLVMGetTypeKind(spTy2) != LLVMPointerTypeKind && LLVMGetTypeKind(spTy2) != LLVMStructTypeKind)
                    {
                        if (verbose_)
                            printf("[expression_codegen] genMemberAccess: unexpected base type kind=%d for '%s', attempting fallback bitcast2\n", (int)LLVMGetTypeKind(spTy2), objVar->name.c_str());
                        LLVMTypeRef desiredPtr2 = LLVMPointerType(structType, 0);
...
LLVMValueRef ExpressionCodeGen::genMethodCall(MethodCallExpr *methodCall)
{
    ...
    if (objType.type == QuarkType::Struct)
    {
        ...
        if (expectedSelfType != LLVMPointerType(objStructTy, 0))
        {
            ...
            if (g_struct_types_)
            {
                for (const auto &p : *g_struct_types_)
                {
                    if (LLVMPointerType(p.second, 0) == expectedSelfType)
                    {
                        expectedStructName = p.first;
                        break;
                    }
                }
            }
            ...
            while (true)
            {
                if (cur == expectedStructName)
                {
                    derives = true;
                    break;
                }
                auto it = g_struct_defs_->find(cur);
                if (it == g_struct_defs_->end() || it->second->parentName.empty())
                    break;
            ...
            if (varExprObj->name == "this" && g_named_values_ && g_function_map_)
            {
                ...
                if (haveDyn)
                {
                    ...
                    for (const auto &p : *g_function_map_)
                    {
                        ...
                        if (mname != methodCall->methodName)
                            continue;
                        std::string cur = sname;
                        bool derives = false;
                        while (true)
                        {
                            if (cur == targetStructName)
                            {
                                derives = true;
                                break;
                            }
                    ...
                    auto genCallForStruct = [&](const std::string &targetStruct)
                    {
                        ...
                        if (g_function_param_types_)
                        {
                            ...
                            if (pti != g_function_param_types_->end())
                            {
                                ...
                                for (size_t i = 0; i < methodCall->args.size(); ++i)
                                {
                                    ...
                                    {
                                        if (isLLVMIntegerType(expected[paramIndex]))
                                            a = genExprIntWithType(methodCall->args[i].get(), expected[paramIndex]);
                                        else if (expected[paramIndex] == bool_t_)
                                            a = genExprBool(methodCall->args[i].get());
                                        else
                            ...
                            {
                                for (size_t i = 0; i < methodCall->args.size(); ++i)
                                    callArgs.push_back(genExpr(methodCall->args[i].get()));
                            }
                        }
                        else
                        {
                        ...
                        if (!isVoid)
                            LLVMBuildStore(builder_, cv, retAlloca);
                        LLVMBuildBr(builder_, mergeBB);
                    };
    ...
    if (objType.type == QuarkType::String)
    {
        ...
        for (const auto &arg : methodCall->args)
        {
            LLVMValueRef argValue = genExpr(arg.get());
            if (!argValue)
                throw std::runtime_error("Failed to generate method call argument");
            args.push_back(argValue);
        }
...
LLVMValueRef ExpressionCodeGen::genStaticCall(StaticCallExpr *staticCall)
{
    ...
    if (!isConstructor)
    {
        ...
        if (g_struct_types_)
        {
            ...
            if (itTy != g_struct_types_->end())
                structTy = itTy->second;
        }
        if (!structTy)
            throw std::runtime_error("unknown struct type '" + actualStructType + "' for static call");
...
QuarkType ExpressionCodeGen::llvmTypeToQuarkType(LLVMTypeRef type)
{
    ...
    if (type == LLVMVoidTypeInContext(ctx_))
...
{
    if (verbose_)
        printf("[codegen] generating address-of expression\n");
    ...
    if (auto *var = dynamic_cast<VariableExprAST *>(addrOf->operand.get()))
    {
        if (g_function_map_)
        {
            ...
            {
                if (verbose_)
                    printf("[codegen] returning address of function: %s\n", var->name.c_str());
                return fit->second;
            }
        }
        ...
        if (verbose_)
            printf("[codegen] returning address of variable: %s\n", var->name.c_str());
        return varAlloca;
    }
...
{
    if (verbose_)
        printf("[codegen] generating dereference expression\n");
    ...
    try
    {
        ...
        case QuarkType::Array:
            ...
            break;
        default:
            if (opT.type == QuarkType::Int)
                elemTy = int32_t_;
            else if (opT.type == QuarkType::Float)
                elemTy = float_t_;
            else if (opT.type == QuarkType::Double)
                elemTy = double_t_;
            else if (opT.type == QuarkType::Boolean)
                elemTy = bool_t_;
            break;
        }
    }
    ...
    if (verbose_)
        printf("[codegen] dereferenced pointer\n");
    return result;
}
...
{
    if (verbose_)
        printf("[codegen] generating range expression\n");

    LLVMValueRef endValue = genExpr(range->end.get());
    if (verbose_)
        printf("[codegen] generated range end value\n");
    return endValue;
}

LLVMValueRef ExpressionCodeGen::genIndirectCall(IndirectCallExpr *indirectCall)
{
    if (verbose_)
...
{
    if (verbose_)
        printf("[codegen] generating array literal with %zu elements\n", arrayLiteral->elements.size());
    ...
    if (verbose_)
        printf("[codegen] generated heap array with header\n");
...
LLVMValueRef ExpressionCodeGen::genMapLiteral(MapLiteralExpr *mapLiteral)
{
    ...
    if (verbose_)
        printf("[codegen] generated map literal\n");
    return mapPtr;
}
...
{
    if (verbose_)
        printf("[codegen] generating array access\n");
    ...
    if (arrayTypeInfo.type == QuarkType::Map)
    {
        ...
        if (verbose_)
            printf("[codegen] generated map subscript access\n");
        return result;
    }
    ...
    {
        if (verbose_)
            printf("[codegen] generating list subscript access\n");
        ...
        if (verbose_)
            printf("[codegen] generated list subscript access\n");
        return result;
    }
    ...
    if (verbose_)
        printf("[codegen] generated array access\n");
    return result;
}
...
{
    if (verbose_)
        printf("[codegen] declaring native map runtime\n");
...
{
    if (verbose_)
        printf("[codegen] declaring native list runtime\n");
...
LLVMValueRef ExpressionCodeGen::genListLiteral(ListLiteralExpr *listLiteral)
{
    ...
    if (verbose_)
        printf("[codegen] generated list literal\n");
    return listPtr;
}