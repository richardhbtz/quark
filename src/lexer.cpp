...
Token Lexer::next()
{
    ...
    if (isalpha((unsigned char)c) || c == '_')
    {
        ...
        else if (s == "if") tok.kind = tok_if;
        else if (s == "elif") tok.kind = tok_elif;
        else if (s == "else") tok.kind = tok_else;
        else if (s == "while") tok.kind = tok_while;
        else if (s == "ret") tok.kind = tok_ret;
        else if (s == "true") {
            tok.kind = tok_true;
            tok.numberValue = 1.0;
        }
        else if (s == "false") tok.kind = tok_false;
        else if (s == "bool") tok.kind = tok_bool;
        else if (s == "int") tok.kind = tok_int;
        ...
        else if (s == "impl") tok.kind = tok_impl;
        else if (s == "data") tok.kind = tok_data;
        else if (s == "extend") tok.kind = tok_extend;
        else if (s == "this") tok.kind = tok_this;
    ...
    if (src_[idx_] == '0' && idx_ + 1 < src_.size() && (src_[idx_ + 1] == 'x' || src_[idx_ + 1] == 'X')) {
            // consume '0x'
            advancePosition(src_[idx_]); ++idx_;
            advancePosition(src_[idx_]); ++idx_;
            size_t hexStart = idx_;
            while (idx_ < src_.size()) {
                char ch = src_[idx_];
                if (isxdigit((unsigned char)ch)) { advancePosition(ch); ++idx_; continue; }
                break;
            }
